...
/**
 * Register Url Notification
 */
function register_my_custom_menu_page(){
    add_submenu_page( 'options.php','', '', 'manage_options', 'wordpresswoocommercepagseguro/wpwcpagseguro_notification.php' );
}
    ...
    class WP_WC_PagSeguro_Gateway extends WC_Payment_Gateway {
         ...
         */
         public function __construct() {
            ...
            //Insert into db the PagSeguro status
            $modal_pagseguro->saveOrderStatus();
        }
        ...
        public function admin_options() {
            ...
            if ( ! $this->is_valid_for_use() ) {
                ...
                // Valid currency.
                echo '<div class="inline error"><p><strong>' . __( 'Gateway Disabled', 'wpwcpagseguro' ) . '</strong>: ' . __( 'PagSeguro does not support your store currency.', 'wpwcpagseguro' ) . '</p></div>';
        ...
        public function init_form_fields() {
            ...
            $this->form_fields = array(
                'enabled' => array(
                    ...
                    'type' => 'checkbox',
                    'label' => __( 'Enable the module?', 'wpwcpagseguro' ),
                    'default' => 'yes'
                ),
                'title' => array(
                    'title' => __( 'Title', 'wpwcpagseguro' ),
                    'type' => 'text',
                    'description' => __( 'Title of checkout.', 'wpwcpagseguro' ),
                    'default' => __( 'PagSeguro', 'wpwcpagseguro' )
                ),
                'description' => array(
                    'title' => __( 'Description', 'wpwcpagseguro' ),
                    'type' => 'textarea',
                    'description' => __( 'Description of checkout.', 'wpwcpagseguro' ),
                    'default' => __( 'Pay via PagSeguro', 'wpwcpagseguro' )
                ),
                'email' => array(
                    'title' => __( 'Email', 'wpwcpagseguro' ),
                    'type' => 'text',
                    'description' => __( 'Do not have a PagSeguro account? <a href="https://pagseguro.uol.com.br/registration/registration.jhtml?ep=10&tipo=cadastro#!vendedor" target="_blank">Click here </a> and register for free.', 'wpwcpagseguro' ),
                    'default' => ''
                ),
                'token' => array(
                ...
                'url_redirect' => array(
                    'title' => __( 'Url Redirect', 'wpwcpagseguro' ),
                    'type' => 'text',
                ...
                'url_notification' => array(
                    'title' => __( 'Url Notification', 'wpwcpagseguro' ),
                    'type' => 'text',
                    'description' => __( 'Whenever a transaction change its status, PagSeguro sends a notification to your store or to the URL entered in this field.', 'wpwcpagseguro' ),
                    'default' => ''
                ),
                'invoice_prefix' => array(
                    'title' => __( 'Invoice Prefix', 'wpwcpagseguro' ),
                    'type' => 'text',
                    'description' => __( 'Please enter a prefix for your invoice numbers.', 'wpwcpagseguro' ),
                    'default' => 'WC-'
                ),
                'charset' => array(
                    ...
                    if ( $item['qty'] ) {
                        ...
                        foreach ( $woocommerce->cart->cart_contents as $cart_item_key => $values ) {
                            ...
                            if($_product->id == $item['product_id']){
                                $pagSeguroItem->setWeight($_product->get_weight());
                                break;
                            }
                        }
        ...
        public function payment($order){
            ...
            if(!empty($this->url_notification)){
                ...
                $paymentRequest->setNotificationURL(home_url().'/wp-admin/options.php?page=wordpresswoocommercepagseguro/wpwcpagseguro_notification.php');
            }
            ...
            $paymentRequest->setExtraAmount(($order->order_discount + $order->cart_discount) * -1);
            
            try {
                $credentials = new PagSeguroAccountCredentials($this->email, $this->token);
                return $paymentRequest->register($credentials);
            } catch (PagSeguroServiceException $e) {
                wp_die( __( 'Sorry, unfortunately there was an error during checkout. Please contact the store administrator if the problem persists.', 'wpwcpagseguro' ) );
            }
        }
        ...
        public function process_payment( $order_id ) {
            ...
            $array_order = $modal_pagseguro->getOrderStatus('pt_BR');
            $key = $modal_pagseguro->getKeyOrderStatusByName($array_order[1][0]);
        ...
        public function mail_missing_message() {
            $html = '<div class="error">';
                $html .= '<p>' . sprintf( __( '<strong>Gateway Disabled</strong> You should inform your email address in PagSeguro.', 'wpwcpagseguro' ), '<a href="' . get_admin_url() . 'admin.php?page=woocommerce_settings&amp;tab=payment_gateways">', '</a>' ) . '</p>';
            $html .= '</div>';
        ...
        public function token_missing_message() {
            $html = '<div class="error">';
                $html .= '<p>' .sprintf( __( '<strong>Gateway Disabled</strong> You should inform your token in PagSeguro.', 'wpwcpagseguro' ), '<a href="' . get_admin_url() . 'admin.php?page=woocommerce_settings&amp;tab=payment_gateways">', '</a>' ) . '</p>';
            $html .= '</div>';
