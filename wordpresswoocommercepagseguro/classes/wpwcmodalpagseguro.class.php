...
class WP_WC_Modal_Pagseguro {
    ...
    private static $array_order_status = array(
        'en_US' => array(
	0 => array(0=>'Pending',1=>'Pending'),
        1 => array(0=>'Awaiting payment',1=>'Awaiting_Payment'),
        2 => array(0=>'Processing',1=>'Processing'),
        3 => array(0=>'Paid',1=>'Paid'),
        4 => array(0=>'Complete',1=>'Complete'),
        5 => array(0=>'Dispute',1=>'Dispute'),
        6 => array(0=>'Refunded',1=>'Refunded'),
        7 => array(0=>'Canceled',1=>'Canceled')),
        
        'pt_BR' => array(
        0 => array(0=>'Iniciada',1=>'Iniciada'),
        1 => array(0=>'Aguardando pagamento',1=>'Aguardando_Pagamento'),
        2 => array(0=>'Em análise',1=>'Em_Análise'),
        3 => array(0=>'Paga',1=>'Paga'),
        4 => array(0=>'Disponível',1=>'Disponível'),
        5 => array(0=>'Em disputa',1=>'Em_Disputa'),
        6 => array(0=>'Devolvida',1=>'Devolvida'),
        7 => array(0=>'Cancelada',1=>'Cancelada'))
        );
    
    /**
     * Save PagSeguro status not registered to locale
     */
    public function saveOrderStatus(){
        $this->runsOrderStatus('pt_BR');
        $this->runsOrderStatus('en_US');
    }
    
    /**
     * Save PagSeguro status not registered
     * 
     * @global type $wpdb
     * @param type $locale
     */
    private function runsOrderStatus($locale){
        global $wpdb;
        
        foreach (self::$array_order_status[$locale] as $key => $value){
            $term = $wpdb->get_row( $wpdb->prepare( "SELECT t.* FROM $wpdb->terms AS t WHERE t.name = '".trim($value[0])."'") );
            if ( !$term ){
                $wpdb->query("INSERT INTO $wpdb->terms (name, slug, term_group) VALUES ('".trim($value[0])."','".trim($value[1])."', 0)");

                $term_id = $wpdb->get_var($wpdb->prepare("SELECT term_id FROM $wpdb->terms WHERE name = '".trim($value[0])."'"));

                $wpdb->query("INSERT INTO $wpdb->term_taxonomy (term_id, taxonomy, description, parent, count) VALUES ($term_id,'shop_order_status','',0,0)");
            }
        }
    }
     ...
     * @return Array 
     */
    public function getOrderStatus($locale){
        return self::$array_order_status[$locale];
    }
    ...
    public function saveHistoric($order_id,$name_order_status,$update){
        if($update){
            ...
            $comment_author_url 	= '';
            $comment_content 		= sprintf( __( 'Status updated to %s', 'wpwcpagseguro' ), $name_order_status );
